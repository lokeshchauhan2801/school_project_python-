...
class Annual_marks(Frame): 
    def __init__(self,master):
        ...
        self.t2=Entry(self)
        self.t3=Entry(self)
        self.b1=Button(self,text="Submit",command=self.annual_exam)
    ...
    def annual_exam(self):
        con=connect(db='ankit',user='root',password='Mjstar@143',host='localhost')
        cur=con.cursor()
        roll_no=int(self.t1.get())
        sub_na=self.t2.get()
        marks=int(self.t3.get())
        i=cur.execute("insert into student_exam_details values(%d,'%s','%s',%d)"%(roll_no,'Annual_exam',sub_na,marks))
        con.commit()
        con.close()
        if(i==1):
